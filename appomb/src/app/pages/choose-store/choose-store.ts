...
import { DeliveryMethod, ILastRequest, IUserProfile, IUserSettings } from '../../shared/interfaces';
...
export class ChooseStore implements OnInit {
  stores: Store[] = [];
  ...
  get subtitle() {
    return this.loadingHelper.isLoadingAny() ? '' : 'Receba em casa ou retire nas lojas!';
  }
  ...
  ionViewWillEnter() {
    if (this.afterCurrentStoreDeleted) {
      this.trans.get(['CURRENT_STORE_WAS_REMOVED', 'TITLE_CURRENT_STORE_WAS_REMOVED']).toPromise().then((val) => {
        this.alertHelper.show(val.TITLE_CURRENT_STORE_WAS_REMOVED, val.CURRENT_STORE_WAS_REMOVED);
      });
    }
  ...
  chooseStore(store: Store, autoSelect = false) {
    ...
    console.log(store);
    if (store) {
      const event = new TrackEvent(TrackHelper.EVENTS.STORE_CHOOSE, {'store_id': store.id});
  ...
  getRedirectFunc(store: Store, autoSelect = false): Promise<any> {
    ...
    if (this.user.is_store_seller && this.user.stores_seller_ids.indexOf(store.id) !== -1) {
      return this.router.navigate(['/HomePage']);
    }
    ...
    if (store.store_type == StoreTypeEnum.DELIVERY) {
      if (this.profile) {
        ...
        if (!this.profile.has_address_valid || !this.deliveryState.deliveryAddress) {
          return this.createRedirectToChooseAddressPromise();
        }
      }
    }
  ...
  async redirectSelectStore(store: Store, autoSelect = false) {
    this.getRedirectFunc(store, autoSelect)
      .then(() => {
  ...
  loadStores() {
    console.log('Carregando a lista de lojas...');
    this.loadingHelper.setLoading('storesList', true);
    this.storeService.getStores(this.cityId, this.storeType, this.deliveryType).subscribe(
      (res) => {
  ...
  mustShowStoreList() {
    return this.stores && ((this.ignoreAmountStoresHideLoading || this.stores.length > 1) || this.afterCurrentStoreDeleted) && !this.loading();
  }

  loading() {
    return this.loadingHelper.isLoading('setSettings') || this.loadingHelper.isLoading('storesList')
  }
  ...
  cancelReorder() {
    this.trans.get(['CANCEL', 'CANCEL_REORDER']).subscribe((val) => {
      this.alertHelper.confirm(
        ...
        if (isConfirmed) {
          this.reorder = null;
          this.toastHelper.show({message: 'Alterações CANCELADAS', duration: 2000});
          this.loadStores();
        }
      });
    });
  ...
  saveReorder() {
    ...
    this.adminStoreService.changeStoresOrder(parseStoresToReorder(stores))
      .subscribe(() => {
        this.loadingHelper.hide();
        this.toastHelper.show({message: 'Ordem alterada com sucesso', cssClass: 'toast-success', duration: 1500});
        this.reorder = null;
