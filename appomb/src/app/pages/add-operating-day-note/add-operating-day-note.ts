...
export class AddOperatingDayNotePage implements OnInit {
  ...
  async init(noteType?) {
    ...
    const routeParams = this.route.snapshot.queryParams;
     this.day = moment(routeParams['day'] || undefined).toDate();
  ...
  ngOnInit() {
    this.init(this.noteTypeEnum.NORMAL);
  }
  ...
  loadNoteOfDay() {
    ...
    this.operatingDaysNoteService
      .getDay(this.store.id, moment(this.day), true, false)
      .subscribe((res) => {
        this.notes = res;
  ...
  changeNoteType(notes: OperatingDaysNote[], noteType: NoteTypeEnum) {
    ...
    this.currentNote = OperatingDayNoteUtil.getByType(
      currentNoteList.filter((item) => item.is_visible),
      noteType
    );
  ...
  onSegmentChanged(event) {
    this.changeNoteType(this.notes, event.detail.value);
  }
  ...
  buildForm(noteType: NoteTypeEnum): void {
    if (Utils.isNullOrUndefined(this.form)) {
      this.form = this.fb.group({
        ...
        repeat_period: [this.periods[0].value, [Validators.required]],
        popup_type: [this.popupTypes[0].value, [Validators.required]],
        app_version_to_update: ['', [Validators.required]],
        message: ['', [Validators.required]],
        title: ['', [Validators.required]],
        showAllOpen: false,
        image: [''],
        note_type: [noteType, [Validators.required]],
      });
    }
  ...
  createNote() {
    ...
    if (this.form.valid) {
      ...
      if (this.noteType === NoteTypeEnum.WARNING) {
        ...
        data.app_version_to_update =
          this.form.controls['app_version_to_update'].value || null;
      ...
      this.operatingDaysNoteService.create(data).subscribe(() => {
        ...
        this.trans
          .get('MSG_ADDED')
          .subscribe((val) => this.toastHelper.show({ message: val }));
  ...
  deleteNote(note: OperatingDaysNote) {
    this.trans
      .get(['CONFIRM_ACTION', 'CONFIRM_ACTION_REMOVE_MSG'])
      .subscribe((val) => {
        this.alertHelper
          .confirm(val.CONFIRM_ACTION, val.CONFIRM_ACTION_REMOVE_MSG)
          .then((confirmed) => {
            if (confirmed) {
              this.loadingHelper.show();
              this.operatingDaysNoteService.delete(note.id).subscribe(
                () => {
                  this.loadingHelper.hide();
