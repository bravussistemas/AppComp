...
import {
  CouponInvalidError,
  CouponUserRegister,
} from '../../providers/coupons.service';
import { AdminStoreService } from '../../providers/admin-store.service';
import {
  DeliveryState,
  DeliveryStateData,
} from '../../providers/delivery-state/delivery-state';
import {
  AddressDeliveryInfo,
  DeliveryHourSimple,
  UserAddressProvider,
...
import {
  CreditCardService,
  IPurchaseItemData,
} from '../../providers/credit-card.service';
...
function getMoreSoonPossibleDeliveryHour(
  dateLimit: Date,
  data: DeliveryHourSimple[],
  invalidIds: number[]
): DeliveryHourSimple {
  if (!data) {
    return null;
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (
      invalidIds &&
      invalidIds.length &&
      invalidIds.indexOf(deliveryHourSimple.id) !== -1
    ) {
      continue;
    }
    if (!first) {
...
export class CheckoutComplete implements OnInit, OnDestroy {
  ...
  get couponDiscountPrice() {
    if (this.canApplyCoupon) {
      const amountDiscount =
        parseFloat(this.couponRegister.coupon.amount_discount) || 0;
      if (this.couponRegister.coupon.is_percentage) {
        return parseFloat(
          (this.productsPrice * (amountDiscount / 100)).toFixed(2)
        );
      } else {
  ...
  startTimer() {
    var counter = setTimeout(() => {
      ...
      this.pix_timer = time;
      if (this.pix_remainingTime < 0) return;
  ...
  clicoucopiar() {
    if (!this.platform.is('cordova')) {
      this.toastHelper.show({
        message: 'Plataforma não permite copiar, copie de forma manual!',
        duration: 1500,
      });
      return;
    } else {
  ...
  get deliveryPriceCouponDiscount() {
    if (this.canApplyCoupon) {
      const amountDiscount =
        parseFloat(this.couponRegister.coupon.amount_discount_delivery_price) ||
        0;
      const di = this.deliveryAddressInfo;
      if (di && !this.isFreeDeliveryByPrice && di.delivery_price) {
        return parseFloat(
          (di.delivery_price * (amountDiscount / 100)).toFixed(2)
        );
      }
    }
    return 0;
  ...
  get mustShowChooseDeliveryHour() {
    return (
      this.store &&
      this.store.user_can_choose_delivery_hour &&
      this.isForDelivery
    );
  }
  ...
  get isDocumentNoteRequired() {
    ...
    if (
      this.store.document_note_option_required === 'delivery' &&
      !this.isForDelivery
    ) {
      return false;
    }
    if (
      this.store.document_note_option_required === 'store' &&
      !this.isForDelivery
    ) {
      return true;
    }
  ...
  loadDeliveryAddressInfo(address: Address, store: Store) {
    ...
    this.userAddressProvider.getDeliveryInfo(address, store.id).subscribe(
      (data) => {
  ...
  get canApplyCoupon() {
    return (
      ...
      (this.couponRegister.coupon.amount_discount ||
        this.couponRegister.coupon.amount_discount_delivery_price) &&
      this.productsPrice &&
      this.productsPrice > this.minimunCoupon
    );
  }
  ...
  get minimunCoupon() {
    return (
      parseInt(
        this.couponRegister &&
          this.couponRegister.coupon &&
          this.couponRegister.coupon.min_sale_price,
        10
      ) || 0
    );
  }
  ...
  notifyNotHasDeliveryHour() {
    this.alertHelper.show(
      'Sem horário para entrega disponível',
      'Já encerramos as entregas ou todos os horários estão preenchidos. Confira nossos horários clicando em "Horário para entrega"'
    );
  }
  ...
  calculateDeliveryHour(data?: AddressDeliveryInfo) {
    ...
    this.cartManagerTable.getMoreLateNextBach().then((date) => {
      ...
      if (data) {
        possibleDeliveryHour = getMoreSoonPossibleDeliveryHour(
          date,
          data.delivery_hours,
          data.invalid_dh_ids
        );
  ...
  cancelDay(day) {
    this.trans
      .get(['CANCEL_RESERVATION_ITEMS', 'WANT_CANCEL_RESERVATION'])
      .toPromise()
      .then((result) => {
        this.alertHelper
          .confirm(
            result.CANCEL_RESERVATION_ITEMS,
            result.WANT_CANCEL_RESERVATION
          )
          .then((confirmed) => {
            if (confirmed) {
      ...
      (resp) => {
        ...
        if (this.firstInit) {
          ...
          if (this.inSellerMode(user, store)) {
            ...
            this.creditCardService.getDefault().subscribe(
              (resp) => {
  ...
  loadDeliveryInfo(store: Store) {
    ...
    this.deliveryStateSub = this.deliveryState.state$.subscribe(
      (state) => {
        if (!state) {
          ...
          if (
            state.address &&
            state.method == DeliveryMethod.DELIVERY_METHOD_HOUSE
          ) {
  ...
  ngOnInit() {
    ...
    this.auth.getUser().then((user) => {
      ...
      this.subscriptions.push(
        this.events
          .onEvent('creditCard')
          .subscribe((data) => this.eventCreditCardChanged(data)),
        this.events
          .onEvent('loadCouponsList')
          .subscribe(() => this.loadCouponsEvent()),
        this.events
          .onEvent('couponDeleted')
          .subscribe(() => this.loadCouponsEvent()),
        this.events
          .onEvent('documentNoteUpdated')
          .subscribe((data) => this.documentNoteUpdatedEvent(data)),
        this.events
          .onEvent('scheduleDeliveryDateChanged')
          .subscribe((data) => this.scheduleDeliveryDateChanged(data))
      );
        ...
        this.settingsService
          ...
          .then((resp) => {
            ...
            this.subscriptions.push(
              this.events
                .onEvent('cartChanged')
                .subscribe((data) => this.eventCartChangedCheckout(data)),
              this.events
                .onEvent('cartChanged')
                .subscribe(() => this.eventUpdateReservationInfo())
            );
  ...
  setCardList(cardList: ICreditCard[]) {
    ...
    if (cardList && cardList.length) {
      ...
      if (this.defaultCard.card_number) {
        this.defaultCard.card_number = Utils.reduceCardMask(
          this.defaultCard.card_number
        );
      }
    }
  }
  ...
  async onSuccessPay(sale: Sales) {
    try {
      ...
      if (this.total) {
        this.trackHelper.trackByName(TrackHelper.EVENTS.BUY_SUCCESS_DONE, {
  ...
  doPayAction() {
    this.loadingHelper.show(
      'Enviando seu pedido... <br> Por favor, não feche o app ainda ;)'
    );
    this.cartManagerTable
      .getCartInfo()
      .then((data) => {
        this.cartManagerTable.getFuture().then((rows) => {
          ...
          if (this.productsPrice != data.totalToPay) {
            ...
            this.alertHelper.show(
              'Erro',
              'Problema ao registrar informações da compra, tente novamente.'
            );
          ...
          if (this.isForDelivery && this.deliveryState) {
            if (
              ...
              this.deliveryState.deliveryAddress.id
            ) {
              deliveryAddressId = this.deliveryState.deliveryAddress.id;
            }
            if (
              this.deliveryState.deliveryHour &&
              this.deliveryState.deliveryHour.id
            ) {
              deliveryHourId = this.deliveryState.deliveryHour.id;
            }
            deliveryAreaId = this.deliveryAddressInfo
              ? this.deliveryAddressInfo.delivery_area
              : null;
          }
          if (this.scheduleDeliveryDate && this.scheduleDeliveryDate.date) {
            if (this.scheduleDeliveryDate.date instanceof Date) {
              delivery_schedule_date = moment(
                this.scheduleDeliveryDate.date
              ).format('YYYY-MM-DD');
            } else {
          ...
          if (
            this.scheduleDeliveryDate &&
            this.scheduleDeliveryDate.observation
          ) {
          ...
          if (this.inSellerMode(this.user, this.store)) {
            ...
            this.handlePayRequest(
              this.adminStoreService.createSale({
                price: totalToPay,
                store: this.store.id,
                items: items,
            ...
            if (this.tipopagamento == 'pix') chosedcard = null;
            else chosedcard = this.defaultCard ? this.defaultCard.id : null;

            this.handlePayRequest(
              this.creditCardService.createPurchase({
                version: 2,
                card: chosedcard,
                price: totalToPay,
                store: this.store.id,
  ...
  get canConfirmRequest() {
    ...
    else liberadoformapagamento = true;
    return Boolean(
  ...
  get mustSelectCreditCard() {
    if (this.tipopagamento == 'cartao')
      return (
        !this.defaultCard &&
        !this.useCredit &&
        !this.inSellerMode(this.user, this.store)
      );
    else return false;
  }

  async handlePayRequest(payRequest: Observable<any>) {
    payRequest.subscribe(
      async (sale: Sales) => {
  ...
  get mustSelectDeliveryAddress() {
    return Boolean(
      this.isForDelivery &&
        (!this.deliveryStateData || !this.deliveryStateData.address)
    );
  }
  ...
  get mustSelectDeliveryDate() {
    const enabled = Boolean(
      this.store.show_schedule_delivery_option &&
        this.store.schedule_delivery_option_required
    );
    return Boolean(
      enabled && (!this.scheduleDeliveryDate || !this.scheduleDeliveryDate.date)
    );
  }
  ...
  payAction() {
    if (this.user && this.inSellerMode(this.user, this.store)) {
      if (this.selectedFinishMethod === null) {
        this.toastHelper.show({
          message: 'Escolha o médoto de pagamento.',
          duration: 1500,
        });
        return;
      }
    ...
    if (
      ...
      this.router
        .navigate(['/register-credit-card'], {
          queryParams: { redirectAfter: 'CheckoutComplete' },
        })
        .then(() => {
          this.toastHelper.show({
            message: 'Cadastre um cartão para finalizar a sua compra.',
          });
        })
        .catch((error) => {
    ...
    if (!this.storewithpix && this.tipopagamento == 'pix') {
      this.toastHelper.show({
        message: 'A loja escolhida não possui pagamento por pix',
      });
      return;
    }
    ...
    if (this.mustAddDocumentNote) {
      this.goToRegisterDocumentNote().then(() => {
        this.toastHelper.show({
          message: 'Cadastre um documento para a geração do cupom fiscal.',
        });
      });
      return;
    }
    if (this.total) {
      this.trackHelper.trackByName(TrackHelper.EVENTS.FINISH_BUY_CLICKED, {
        total: this.total.toString(),
      });
    }
    ...
    if (this.isForDelivery) {
      ...
      this.trans
        .get(['TITLE_CONFIRM_STORE_BUY', 'CHANGE_STORE', 'CONFIRM'])
        .subscribe((val) => {
          this.alertHelper
            .confirm(
            ...
            .then((confirmed) => {
              if (confirmed) {
                ...
                this.trackHelper.trackByName(
                  TrackHelper.EVENTS.CANCEL_BUY_TO_CHANGE_STORE
                );
  ...
  goToChooseScheduleDeliveryDate() {
    ...
    this.router
      .navigate(['/choose-schedule-delivery-date'], {
        queryParams: {
          date: params.date,
      ...
      .catch((error) => {
        console.error(
          'Erro ao navegar para ChooseScheduleDeliveryDate:',
          error
        );
      });
  }
  ...
  goToChooseDefaultCard() {
    ...
    if (this.defaultCard) {
      this.router.navigate(['/ListCreditCardPage'], {
        queryParams: { action: 'toChooseDefault' },
      });
    } else {
  ...
  goToChooseCoupon() {
    if (this.couponErrors && this.couponErrors.length) {
      this.router.navigate(['/CouponsPage'], {
  ...
  selectedDeliveryHourValid() {
    ...
    if (
      ...
      !this.deliveryStateData.hour ||
      !this.deliveryStateData.hour.id
    ) {
      return false;
    }
    const deliveryHour = this.deliveryAddressInfo.delivery_hours.find(
      (item) => {
    ...
    if (
      !deliveryHour ||
      deliveryHour.start_hour != this.deliveryStateData.hour.start_hour ||
      deliveryHour.end_hour != this.deliveryStateData.hour.end_hour
    ) {
      return false;
    }
    return (
      this.deliveryAddressInfo.invalid_dh_ids.indexOf(
        this.deliveryStateData.hour.id
      ) === -1
    );
  }
  ...
  handlePayError(error: Response) {
    ...
    if (error instanceof Response) {
      ...
      if (serverResponse.status === 400) {
        this.alertHelper.show(
          'Erro',
          serverResponse.getErrorMessage() || defaultMsg
        );
      } else {
      ...
      this.alertHelper.show(
        'Erro',
        'Ocorreu um erro ao finalizar a sua compra, verifique a sua conexão com a internet.'
      );
    }
  }
  ...
  get isLoading() {
    return (
      (!this.alreadyLoaded && this.loadingHelper.isLoading('checkoutList')) ||
  ...
  get hasCoupon() {
    return (
      this.couponRegister &&
      this.couponRegister.id &&
      this.couponRegister.coupon &&
      this.couponRegister.coupon.id
    );
  }
