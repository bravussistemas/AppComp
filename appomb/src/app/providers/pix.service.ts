...
export class PixService {
  ...
  create(data: IPIXCreationData): Observable<any> {
    return this.authUserHttp
      .post(this.getUrl(this.appConfig.API_PIX), data)
      .pipe(
        map((resp: any) => {
          return resp;
        })
  ...
  get(params): Observable<any> {
    let paramshttp = new HttpParams().set('', params);
    return this.authUserHttp
      .put(this.getUrl(this.appConfig.API_PIX), paramshttp)
      .pipe(
        map((resp: any) => {
          return resp;
        })
  ...
  verifypix(store_id): Observable<any> {
    let paramshttp = new HttpParams().set('store_id', store_id);
    console.log(paramshttp);

    const body = { store_id };

    return this.authUserHttp
      .post(this.getUrl(this.appConfig.API_VERIFYPIX), body)
      .pipe(
        map((resp: any) => {
          console.log(resp);
