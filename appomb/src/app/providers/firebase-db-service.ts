...
export class FirebaseDbService {
   ...
   */
  watchInventoryChanges(): Observable<any[]> { // Replace 'any' with actual inventory type
   ...
   */
  watchDeliveryEndedChanges(): Observable<any> { // Replace 'any' with actual type
   ...
   * @returns An Observable of any type (replace with actual type if available).
   */
  watchOperatingNotesDaysChanges(storeId: number): Observable<any> { // Replace 'any' with actual type
   ...
   */
  listDispatchOrders(day: Moment, storeId: number): Observable<DispatchOrderAdminList[]> {
    const path = `/${this.appConfig.FIREBASE_DB_DISPATCH_ORDERS}/${day.format('YYYYMMDD')}/`;
    return this.db.list<DispatchOrderAdminList>(path, ref =>
