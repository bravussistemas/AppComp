...
import { AlertController, ModalController, Platform, NavController, IonRouterOutlet } from '@ionic/angular';
...
export class AppComponent implements OnDestroy, OnInit {
  ...
  constructor(
    ...
    this.pagesStoreSales = [
      USER_CREDIT,
    ];
    this.pagesDeliveryEmployee = [
      DELIVERY_EMPLOYEE,
    ];
    this.pages = [
      HISTORY_PURCHASE,
  ...
  private async setUpKeyboardListeners() {
    if (this.platform.is('capacitor')) {
      try {
        ...
        this.keyboardWillShowListener = await Keyboard.addListener('keyboardWillShow', (info) => {
  ...
  checkLogin() {
    this.auth.loggedIn(true).then((isLoggedIn) => {
      if (!isLoggedIn) {
  ...
  checkDeviceDate(serverDate: Date) {
    ...
    if (!(today.getDate() === serverDate.getDate() &&
      ...
      this.trackHelper.trackByName(TrackHelper.EVENTS.WRONG_DATE_WARNING, {date: today.toString()});
      this.alertHelper.show('Data do aparelho incompatível!',
        `A data do seu aparelho parece diferente da data que operamos em nossas lojas,
              ...
              <hr>
              <b>Data do aparelho:</b> ${this.datePipe.transform(today, 'shortDate')}<br>
              <b>Data das nossas lojas:</b> ${this.datePipe.transform(serverDate, 'shortDate')}`)
    }
  }

  updateSettings() {
    this.settingsService.refreshSettings({with_dispatch_info: true})
      .then(async (res) => {
        ...
        if (!res.store) {
          ...
          return await this.settingsService.updateSettings({ store: settings.store })
            .then(() => {
        ...
        this.storage.get('GO_TO_AFTER_LOGIN').then((val) => {
  ...
  async setInitialRootPage(page: string, isRetry: boolean = false): Promise<void> {
    try {
      const currentUrl = this.router.url;
  ...
  private normalizeUrl(url: string): string {
    ...
    return urlWithoutQuery.endsWith('/') ? urlWithoutQuery.slice(0, -1) : urlWithoutQuery;
  }
  ...
  openPage(page: any, asRoot = true) {
    ...
    this.trackHelper.trackByName(`${TrackHelper.EVENTS.OPEN_PAGE} ${page.toString()}`);
  ...
  watchFirebaseChanges() {
    this.allSubs.add(this.firebaseService.watchStoresChanges().subscribe((e: IFirebaseStoresDb) => {
        ...
        this.storage.get('LAST_STORE_UPDATED').then((val) => {
          this.settingsService.getSettings()
            .then((result: IUserSettings) => {
              if (e.storeIds && result.store_id && e.storeIds.indexOf(result.store_id) === -1) {
                // user store was removed by an admin, redirect user to select a new store.
                this.navController.navigateRoot(FIRST_PAGE_APP, {queryParams:{afterCurrentStoreDeleted: true}});
              }
            }, (error) => {
    ...
    this.settingsService.getSettings()
      .then((result: IUserSettings) => {
        if (result.store_id) {
          this.allSubs.add(this.firebaseService.watchInventoryChanges().subscribe((resp) => {
  ...
  watchDeliveryEndedChanges(user: User) {
    let isFirst = true;
    this.allSubs.add(this.firebaseService.watchDeliveryEndedChanges().subscribe((e: any) => {
        if (isFirst) {
          isFirst = false;
          return;
        }
        ...
        if (fromStore && fromStore.users) {
          if (fromStore.users.indexOf(user.id) !== -1) {
            this.alertHelper.show('Entregador finalizou última entrega!',
  ...
  share() {
    Share.share({text:`Oi, veja que legal o aplicativo dessa panificadora artesanal de fermentação natural aqui em Joinville 👉 ${this.appConfig.APP_ONE_LINK}`}      
    );
  ...
  contact() {
    if(!this.platform.is('cordova'))
      ...
      cordova.InAppBrowser.open('https://www.ohmybread.com/home', '_system', 'location=yes');    
  }
  ...
  handlerError(e: any): void {
    ...
    if (Utils.equalString(this.router.url, '/dummy')) { // Replace '/dummy' with the actual route of 'DummyPage'
      this.setInitialRootPage(this.homePage);
    }
  }
